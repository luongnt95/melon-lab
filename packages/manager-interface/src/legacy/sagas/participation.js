...
function* redeemParosSharesSaga(action) {
  function* transaction(environment) {
    ...
    yield put(actions.redeemParosSharesSucceeded());
    yield put(modalActions.close());
  }

  yield call(
    signer,
    `Please confirm you wish to redeem the underlying assets of your fund to your manager address.`,
    transaction,
    actions.redeemParosSharesFailed,
  );
}
