...
const buildTwitterUrl = (isOwner, fundAddress, fundName, sharePrice, track) => {
    const text = isOwner
        ? track !== "live" ? `My #MelonFund "${fundName}" has a share price currently of ${displayNumber(
            sharePrice,
        )}. Have a look:` : `Check out my on-chain decentralized hedge fund "${fundName}". It currently has a share price of ${displayNumber(
            sharePrice,
        )}. Have a look:`
        : track !== "live" ? `The #MelonFund "${fundName}" has a share price currently of ${displayNumber(
            sharePrice,
        )}. Have a look:` : `Check out this on-chain decentralized hedge fund "${fundName}". It currently has a share price of ${displayNumber(
            sharePrice,
        )}. Have a look:`;

    const url = track === "live" ? `https://ipfs.io/ipns/olympiad.melon.fund/#${fundAddress}` : `https://ipfs.io/ipns/melon.fund/#${fundAddress}`;
